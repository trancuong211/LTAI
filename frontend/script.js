...
function getFormData() {
  ...
  const data = {
    ...
    khoang_cach_trung_tam: parseFloat(
...
async function predict() {
  ...
  // set loading state on button (save original content to restore later)
  try {
  ...
  btn.disabled = true;
  // disable all inputs inside form-section for accessibility
  ...
  // Show skeleton inside the result box and keep the result container visible
  ...
  try {
    ...
    const giaPerM2 =
      priceBillion != null && data.dien_tich > 0
        ? ((priceBillion * 1000) / data.dien_tich).toLocaleString("vi-VN", {
    ...
    // attach back button handler (idempotent)
    ...
    // restore button content and enable inputs
    try {
